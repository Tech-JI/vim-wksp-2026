...
| `^` | 行首 | `/^foo` 匹配以 `foo` 开头的行 |
| `$` | 行尾 | `/bar$` 匹配以 `bar` 结尾的行 |
| `.` | 任意单个字符 | `/foo.bar` 匹配 `fooXbar` |
| `\d` | 数字 | `/\vitem-\d+` 匹配 `item-42` |
| `\a` | 字母 | `/\v\a+` 匹配连续字母 |
| `\w` | word 字符，字母、数字或下划线 | `/\v\w+` 匹配一个 word |
| `\s` | 空白字符 | `/\vfoo\s+bar` 匹配中间有空白的 `foo bar` |
| `*` | 重复 0 次或多次 | `/\vba*` 匹配 `b`、`ba`、`baaa` |
| `+` | 重复 1 次或多次 | `/\v\d+` 匹配连续数字 |
| `?` | 重复 0 次或 1 次 | `/\vcolou?r` 匹配 `color` 或 `colour` |
| `{n,}` | 重复至少 n 次 | `/\v\d{3,}` 匹配至少 3 位数字 |
| `{,n}` | 重复至多 n 次 | `/\v\d{,4}` 匹配至多 4 位数字 |
| `{n,m}` | 重复 n 到 m 次 | `/\v\d{2,4}` 匹配 2 到 4 位数字 |
| `(foo\|bar)` | 分组和或 | `/\v(foo\|bar)` 匹配 `foo` 或 `bar` |
| `\1` | 第 1 个捕获组 | `:%s/\vitem-(\d+)/item #\1/g` |
...
1. 运行 `:help magic`。
2. 找到帮助文档里的 magic 规则表。
3. 把这张表复制到当前文件的临时位置。
4. 给每一行添加 Markdown 表格符 `|`。
5. 添加表头分隔行，例如 `| --- | --- | --- | --- | --- |`。
6. 检查表格列是否对齐。
